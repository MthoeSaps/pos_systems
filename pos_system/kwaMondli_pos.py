...
def save_transaction(customer_name, item_name, item_price, item_quantity, total, payment_method):
    ...
    if not os.path.exists("pos_system\dbs\kwaMondli_pos_transactions.csv"):
        df.to_csv("pos_system\dbs\KwaMondli_pos_transactions.csv", index = False)
    else:
        df.to_csv("pos_system\dbs\KwaMondli_pos_transactions.csv", mode="a", index=False, header=not os.path.exists("E:\pos_system\dbs\pos_transactions.csv"))
        st.success("Transaction Saved Succesfully!", icon ="🅿️")
        
def display_inventory():
    """Function to display inventory system"""
    #__read the csv file
    if os.path.exists("pos_system\dbs\KwaMondli_pos_transactions.csv"):
        df = pd.read_csv("pos_system\dbs\KwaMondli_pos_transactions.csv")
